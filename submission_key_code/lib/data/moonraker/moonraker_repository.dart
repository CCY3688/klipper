...
class MoonrakerRepository {
  ...
  Map<String, dynamic> _unwrapResultMap(Map<String, dynamic> resp) {
  ...
  Future<Map<String, dynamic>> buildSubscriptionObjects(
    StatusProfile profile,
  ) async {
    final objectsList = await fetchObjectsList();
    final list =
        (objectsList['result']?['objects'] as List?)?.cast<String>() ??
        const [];
  ...
  Future<Map<String, dynamic>> queryStatusSnapshot(
  ...
  Future<void> runGcodeScript(String script, {Duration? receiveTimeout}) async {
    await http.gcodeScript(script, receiveTimeout: receiveTimeout);
  }

  Future<void> restartKlipperScreen() async {
    try {
      await http.restartService('KlipperScreen');
    } catch (_) {
      await http.restartService('klipperscreen');
    }
  }
  ...
  Future<Map<String, dynamic>> fetchSurfaceScanList() async {
    return _unwrapResultMap(await http.surfaceScanList());
  }

  Future<Map<String, dynamic>> fetchSurfaceScan({String? profile}) async {
    return _unwrapResultMap(await http.surfaceScanGet(profile: profile));
  }

  Future<Map<String, dynamic>> fetchSurfaceScanProgress() async {
    return _unwrapResultMap(await http.surfaceScanProgress());
  }
  ...
  Future<String> uploadGcodeString({
    ...
    final uploadedPath = (item['path'] ?? item['filename'] ?? filename)
        .toString();
    return uploadedPath;
  }
  ...
  Future<List<MoonrakerFileItem>> listGcodeFiles({
    String root = 'gcodes',
  }) async {
