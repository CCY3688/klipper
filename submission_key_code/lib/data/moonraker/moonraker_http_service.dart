...
import 'moonraker_exception.dart';
...
class MoonrakerHttpService {
  ...
  MoonrakerHttpService(MoonrakerConfig config)
    : _dio = Dio(
        BaseOptions(
          //Dio实例初始化，BaseOptions是Dio的配置类，设置基本选项
  ...
  Future<Map<String, dynamic>> postJson(
    ...
    final resp = await _request(
      () => _dio.post(path, data: body, options: options),
    );
    return (resp.data as Map).cast<String, dynamic>();
  ...
  Future<Map<String, dynamic>> deleteJson(String path) async {
    final resp = await _request(() => _dio.delete(path));
    return (resp.data as Map).cast<String, dynamic>();
  }
  ...
  Future<Map<String, dynamic>> restartService(String service) {
    return postJson('/machine/services/restart', {'service': service});
  }

  Future<Map<String, dynamic>> surfaceScanList() {
    return getJson('/printer/surface_scan/list');
  }

  Future<Map<String, dynamic>> surfaceScanGet({String? profile}) {
    return getJson(
      '/printer/surface_scan/get',
      queryParameters: {
  ...
  Future<Map<String, dynamic>> surfaceScanProgress() {
    return getJson('/printer/surface_scan/progress');
  }
  ...
  Future<Response<T>> _request<T>(Future<Response<T>> Function() run) async {
    try {
      ...
      throw MoonrakerApiException.fromDio(e);
    }
  }
}
