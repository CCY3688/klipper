...
class Toolhead {
  ...
  final String? homedAxes; // e.g. "xyz" or "" (empty = not homed)

  const Toolhead({this.position, this.homedAxes});

  Toolhead applyDelta(JsonMap d) => Toolhead(
        position: d.containsKey('position') ? _toDoubleList(d['position']) : position,
        homedAxes: d.containsKey('homed_axes')
            ? (d['homed_axes']?.toString())
            : homedAxes,
      );
}
...
class MotionReport {
  ...
  final double? liveExtruderVelocity;

  const MotionReport({
    this.livePosition,
    this.liveVelocity,
    this.liveExtruderVelocity,
  });

  MotionReport applyDelta(JsonMap d) => MotionReport(
        livePosition: d.containsKey('live_position')
            ? _toDoubleList(d['live_position'])
            : livePosition,
        liveVelocity: d.containsKey('live_velocity')
            ? (d['live_velocity'] as num?)?.toDouble()
            : liveVelocity,
        liveExtruderVelocity: d.containsKey('live_extruder_velocity')
            ? (d['live_extruder_velocity'] as num?)?.toDouble()
...
class PrinterStatus {
  ...
  final MotionReport? motionReport;
  ...
  PrinterStatus applyDelta(JsonMap delta) {
    ...
    final d5 = delta['motion_report'];
    if (d5 is Map) mr = (mr ?? const MotionReport()).applyDelta(d5.cast<String, dynamic>());
