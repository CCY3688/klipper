...
const Object _paperConfigUnset = Object();
...
class PaperConfig {
  ...
  int get cols {
    ...
    if (!_isPositiveFinite(usableWidth) || !_isPositiveFinite(cellSizeMm)) {
      return 1;
    }
    switch (kind) {
  ...
  int get rows {
    ...
    if (!_isPositiveFinite(usableHeight)) return 1;
    switch (kind) {
      case PaperTypeKind.grid:
        // 第一行占用 cellSizeMm，后续每行占用 cellSizeMm + gridRowSpacingMm
        final rowPitch = cellSizeMm + gridRowSpacingMm;
        if (!_isPositiveFinite(cellSizeMm) || !_isPositiveFinite(rowPitch)) {
          return 1;
        }
        if (usableHeight < cellSizeMm) return 0;
        return (1 + (usableHeight - cellSizeMm) / rowPitch).floor().clamp(
          0,
          500,
        );
      case PaperTypeKind.horizontal:
        if (!_isPositiveFinite(lineSpacingMm)) return 1;
        return (usableHeight / lineSpacingMm).floor().clamp(1, 500);
      case PaperTypeKind.letter:
        if (!_isPositiveFinite(lineSpacingMm)) return 1;
        return (usableHeight / lineSpacingMm).floor().clamp(1, 500);
      case PaperTypeKind.blank:
        if (!_isPositiveFinite(cellSizeMm)) return 1;
  ...
  PaperConfig copyWith({
    ...
    Object? customCols = _paperConfigUnset,
    Object? customRows = _paperConfigUnset,
  }) {
    return PaperConfig(
      name: name ?? this.name,
      kind: kind ?? this.kind,
      ...
      customCols: customCols == _paperConfigUnset
          ? this.customCols
          : customCols as int?,
      customRows: customRows == _paperConfigUnset
          ? this.customRows
          : customRows as int?,
    );
  }
  ...
  PaperConfig withA4LongEdgeOnX() {
    if ((pageWidthMm - 210).abs() < 1 && (pageHeightMm - 297).abs() < 1) {
      return copyWith(
        pageWidthMm: 297,
        pageHeightMm: 210,
        customCols: null,
